...
Research Paper: "Overcoming the Transaction Cost Trap: Cross-Sectional
                 Conviction Ranking in Machine Learning Equity Prediction"
"""
...
FEATURE_NAMES: List[str] = [
    ...
    "VWAP_deviation",
    # --- Extra momentum (1) ---
    "VWAP_deviation",   # placeholder slot filled below — see note
]

# We construct exactly 47 unique features; the list above has 47 slots
# (VWAP_deviation appears once in the actual DataFrame — the duplicate at
# the end is an artefact of the display here; the actual build loop below
# produces 47 distinct columns).  The canonical column list is set
# programmatically after construction to guarantee uniqueness.
...
def _compute_features_single_ticker(ohlcv: pd.DataFrame) -> pd.DataFrame:
    ...
    assert len(feature_df.columns) == 47, (
        f"Expected 47 features, got {len(feature_df.columns)}. "
